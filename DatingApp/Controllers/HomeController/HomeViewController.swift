...
class HomeViewController: UIViewController, CLLocationManagerDelegate {
    ...
    /*
        ///Profile Image is saved separately
        func newImageUrl(uid: String, email: String, fullname: String, phone: String, accountTypeIndex: Int, itemImage: UIImageView, completion:@escaping((String?) -> () )) {
           
            guard let userID = Auth.auth().currentUser?.uid else { completion(nil) ; return }
            
            ///Image file name is saved and differentiated by using the id at the end of the name
            let storageRef = Storage.storage().reference().child("\(uid).png")
            
            ///Image is resized here
            guard let newImage = itemImage.image?.resized() else { return }
            
            guard let imageData2 = newImage.pngData() else { return  }
            
            
            
            guard let imageData3 = itemImage.image?.pngData() else { return  }
            
            let megaByte = 1000.0

            
            let imageSizeKB = Double(imageData2.count) / megaByte // ! Or devide for 1024 if you need KB but not kB
            
            let imageSizeKB2 = Double(imageData3.count) / megaByte
            
           ///Image is converted to data here
            guard let imageData = newImage.jpegData(compressionQuality: 1) else { completion(nil) ; return }
